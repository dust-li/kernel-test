...
new_udp_mem='91944   122592  183888'
...
server_sock = []
for i in range(ServerSocketCnt):
        address = ('0.0.0.0', ServerPort + i)
        server_sock.append(socket.socket(socket.AF_INET, socket.SOCK_DGRAM))
        server_sock[i].bind(address)
        print "server rcvbuf: %d" % server_sock[i].getsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF)
...
for i in range(ServerSocketCnt):
        address = ('127.0.0.1', ServerPort + i)
